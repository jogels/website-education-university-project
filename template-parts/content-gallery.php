...
<article class="blog-sec animated fadeInDown">
  ...
  <?php if(get_theme_mod('campus_education_blog_post_content') == 'Full Content'){ ?>
    <?php the_content(); ?>
  <?php }
  if(get_theme_mod('campus_education_blog_post_content', 'Excerpt Content') == 'Excerpt Content'){ ?>
    <?php if(get_the_excerpt()) { ?>
      <div class="entry-content"><p><?php $excerpt = get_the_excerpt(); echo esc_html( campus_education_string_limit_words( $excerpt, esc_attr(get_theme_mod('campus_education_post_excerpt_number','20')))); ?><?php echo esc_html( get_theme_mod('campus_education_button_excerpt_suffix','...') ); ?></p></div>
    <?php }?>
  <?php }?>
  <?php if ( get_theme_mod('campus_education_blog_button_text','Read Full') != '' ) {?>
    <div class="blogbtn">
      <a href="<?php echo esc_url( get_permalink() );?>" class="blogbutton-small hvr-sweep-to-right"><?php echo esc_html( get_theme_mod('campus_education_blog_button_text',__('Read Full', 'campus-education')) ); ?><span class="screen-reader-text"><?php esc_html_e('Read Full','campus-education'); ?></span></a>
    </div>
  <?php }?>
</article>