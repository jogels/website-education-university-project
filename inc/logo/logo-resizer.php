 ...
 */
function campus_education_logo_customize_register( $wp_customize ) {
	...
	$wp_customize->add_control( 'logo_width', array(
		'label'       => esc_html__( 'Logo Resizer','campus-education' ),
		'description'       => esc_html__( 'Forces the logo to have a specified width(max value is 100).','campus-education' ),
...
add_action( 'customize_register', 'campus_education_logo_customize_register' );

/**
 * Add support for logo resizing by filtering `get_custom_logo`
 */
function campus_education_customize_logo_resize( $html ) {
	...
	if ( is_numeric( $size ) && is_numeric( $custom_logo_id ) ) {
		...
		if ( $logo['width'] >= $logo['height'] ) {
			$output = campus_education_min_max( $logo['height'], $logo['width'], $max['height'], $max['width'], $size, $min );
			$img = array(
				'height'	=> $output['short'],
				'width'		=> $output['long']
			);
		// portrait
			...
			$output = campus_education_min_max( $logo['width'], $logo['height'], $max['width'], $max['height'], $size, $min );
			$img = array(
				'height'	=> $output['long'],
				'width'		=> $output['short']
			);
...
add_filter( 'get_custom_logo', 'campus_education_customize_logo_resize' );

/* Helper function to determine the max size of the logo */
function campus_education_min_max( $short, $long, $short_max, $long_max, $percent, $min ){
	$ratio = ( $long / $short );
...
*/
function campus_education_customize_controls_js() {
	wp_enqueue_script( 'campus-education-customizer-controls', get_template_directory_uri() . '/inc/logo/js/customize-controls.js', array( 'jquery', 'customize-preview' ), '201709071000', true );
}
add_action( 'customize_controls_enqueue_scripts', 'campus_education_customize_controls_js' );

/**
 * Adds CSS to the Customizer controls.
 */
function campus_education_customize_css() {
	wp_add_inline_style( 'customize-controls', '#customize-control-logo_size input[type=range] { width: 100%; }' );
}
add_action( 'customize_controls_enqueue_scripts', 'campus_education_customize_css' );

/**
 * Testing function to remove logo_width theme mod
 */
function campus_education_remove_theme_mod() {
	if ( isset( $_GET['remove_logo_size'] ) && 'true' == $_GET['remove_logo_size'] ){
		set_theme_mod( 'logo_width', '' );
	}
}
add_action( 'wp_loaded', 'campus_education_remove_theme_mod' );